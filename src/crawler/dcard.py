...
# Get Class
    query = response.css("#__next > div.bvk29r-0.eFPEdc > div.bvk29r-2.etVvYS > div > div > div > div > article > div.sc-1eorkjw-3.jJbLey > div:nth-child(1) > a")
    class_ = self.clear_html_tags_from_selectors(query)
# Get Content
    query = response.css('#__next > div.bvk29r-0.eFPEdc > div.bvk29r-2.etVvYS > div > div > div > div > article > div.sc-1eorkjw-5.hKSyLQ > div')
...
# Get Times
    query = response.xpath('/html/body/div[1]/div[2]/div[2]/div/div/div/div/article/div[2]/div[2]/text()')
